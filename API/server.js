...
server.post('/login', (req, res) => {
    ...
    if (isAuthenticated({email, password}) === false) {
      const status = 401
      const message = 'email or mat khau khong chinh xac'
...
server.post('/auth/orders', (req, res) => {
  ...
  const exist_iphone_id = db.iphone.findIndex(iphone => iphone.id === productId)

  if(exist_iphone_id === -1) {
    return res.status(401).json({
      status: 401,
  ...
  const order_iphone = db.iphone[exist_iphone_id]
  if(order_iphone.available) {
    let date=new Date()
    const new_order = {
      'ten san pham':db.order_iphone,
      'id': db.orders.length+1,
      'ten khach hang':customerName,
      "quantity": 1,
      "thoi gian dat hang":date.getDay()+1+"-"+date.getMonth()+"-"+date.getFullYear()
    }
